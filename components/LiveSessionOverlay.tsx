...
export const LiveSessionOverlay: React.FC<LiveSessionOverlayProps> = ({ onClose, onTranscriptUpdate, language, systemInstruction, micAccess = true, initialHistory = [] }) => {
        ...
        callbacks: {
          onopen: () => {
            ...
            scriptProcessor.onaudioprocess = (e) => {
              ...
              sessionPromise.then((s: any) => s.sendRealtimeInput({ media: createBlob(data) }));
            };
  ...
  if (isMinimized) {
    return (
      <div 
        onPointerDown={(e) => { 
          // Fix: Property 'setPointerCapture' does not exist on type 'PointerEvent<HTMLDivElement>'.
        ...
        onPointerUp={(e) => { 
          ...
          // Fix: Property 'releasePointerCapture' does not exist on type 'PointerEvent<HTMLDivElement>'.
      ...
      >
        ...
        <button onClick={toggleMute} className={`p-2 rounded-full ${isMuted ? 'text-red-500' : 'text-gray-400 hover:text-white'}`}><Mic size={18} /></button>
        <button onClick={() => setIsMinimized(false)} className="p-2 text-gray-400 hover:text-white"><Maximize2 size={18} /></button>
        <button onClick={handleEnd} className="p-2 text-gray-400 hover:text-red-500"><X size={18} /></button>
      </div>
    );
  }
  ...
  return (
    <div className="!fixed !inset-0 !z-[9999] !h-[100dvh] !w-screen bg-[#05070a] text-white flex flex-col animate-in fade-in duration-300 overflow-hidden">
      ...
      <div className="relative z-10 w-full flex-1 flex flex-col md:flex-row overflow-hidden">
         <div className={`flex flex-col items-center justify-center transition-all duration-700 ${showTranscript ? 'w-full md:w-1/2 p-4' : 'w-full p-8'}`}>
            <div className="relative w-48 h-48 sm:w-80 sm:h-80 flex items-center justify-center mb-6 sm:mb-10">
                {isConnected && <div className="absolute inset-0 rounded-full bg-blue-600/5 blur-3xl scale-[2] opacity-20"></div>}
                <div className={`w-32 h-32 sm:w-56 sm:h-56 rounded-[2.5rem] sm:rounded-[3rem] flex items-center justify-center transition-all duration-500 shadow-2xl overflow-hidden ${isConnected ? 'bg-gradient-to-tr from-blue-700 to-indigo-600 border border-blue-400/30' : 'bg-gray-800 border-2 border-dashed border-gray-700'}`}>
            ...
            <div className="text-center mb-10">
                <h2 className="text-xl sm:text-3xl font-black text-white tracking-tighter uppercase mb-1">{status}</h2>
                <p className="text-[8px] sm:text-[9px] font-bold text-blue-400 uppercase tracking-[0.4em] opacity-50">Neural Stream Active</p>
            </div>
            <div className="flex items-center justify-center gap-6">
                <button onClick={toggleMute} className={`p-5 sm:p-7 rounded-[1.8rem] sm:rounded-[2.5rem] border-2 transition-all shadow-xl active:scale-90 ${isMuted ? 'bg-red-500 border-red-400 text-white' : 'bg-white border-white text-gray-900 hover:bg-blue-50'}`}><Mic size={28} /></button>
                <button onClick={handleEnd} className="p-5 sm:p-7 rounded-[1.8rem] sm:rounded-[2.5rem] bg-red-600 text-white shadow-2xl hover:bg-red-700 transition-all active:scale-90"><RotateCcw size={28} /></button>
            </div>
         </div>

         {showTranscript && (
             <div className="w-full md:w-1/2 h-full bg-[#080b15]/90 backdrop-blur-3xl border-l border-white/5 flex flex-col p-4 sm:p-8 animate-in slide-in-from-right duration-500">
                 ...
                 <div className="flex-1 overflow-y-auto pr-2 space-y-6 no-scrollbar scroll-smooth">
                    {localTranscript.map((msg, idx) => (
                        <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-bottom-1 duration-300`}>
                            ...
                            <div className={`max-w-[90%] px-5 py-3 rounded-[1.2rem] text-xs sm:text-sm font-medium leading-relaxed border ${msg.role === 'user' ? 'bg-[#151928]/80 border-white/5 text-gray-300' : 'bg-[#0a0d18] border-blue-500/10 text-blue-50'}`}>{msg.content}</div>
                        </div>
                    ))}
